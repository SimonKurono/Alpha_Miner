...
def test_feature1_e2e_gdelt_outage_uses_rss_fallback(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    symbols = [f"SYM{i}" for i in range(10)]
    ...
    def fake_gdelt(req, deadline=None):
        return NewsResponse(
            documents=[],
            missing_symbols=req.symbols,
            source="gdelt",
            metadata={"rate_limited": True, "deadline_reached": False},
        )

    def fake_rss(req, symbols, deadline=None):
        docs = [
            NewsDocument(
                symbol=s,
                source="rss",
                title=f"{s} rss headline",
                published_at=datetime(2024, 1, 20, tzinfo=timezone.utc),
                url=f"https://example.com/rss/{s}",
                snippet="sample",
            )
            for s in symbols[:3]
        ]
        missing = [s for s in symbols if s not in {d.symbol for d in docs}]
        return NewsResponse(
            documents=docs,
            missing_symbols=missing,
            source="rss",
            metadata={"deadline_reached": False},
        )

    monkeypatch.setattr(market_agent, "fetch_stooq_prices", fake_prices)
    monkeypatch.setattr(market_agent, "fetch_latest_shares_outstanding", fake_shares)
    monkeypatch.setattr(text_agent, "fetch_sec_filings", fake_sec)
    monkeypatch.setattr(text_agent, "fetch_gdelt_news", fake_gdelt)
    monkeypatch.setattr(text_agent, "fetch_rss_news", fake_rss)

    async def _run():
        runner = InMemoryRunner(agent=build_root_ingestion_workflow(), app_name="test_feature1_fallback")
        session = await runner.session_service.create_session(
            app_name=runner.app_name,
            user_id="u1",
            session_id="s3",
    ...
    state = asyncio.run(_run())
    assert state["ingestion.quality"]["text_symbol_coverage"] >= 0.20

    breakdown_path = Path(state["ingestion.text.coverage_breakdown"])
    assert breakdown_path.exists()
    payload = breakdown_path.read_text(encoding="utf-8")
    assert "rss_docs" in payload
    assert "gdelt_rate_limited" in payload
